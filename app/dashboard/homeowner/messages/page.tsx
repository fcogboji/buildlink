...
export default async function HomeownerMessagesPage() {
  ...
  const rows = jobs
    .map((job) => {
      ...
      const lastReadAt = byJobId.get(job.id);
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-stone-600">
        Per-job threads with your matched builders.
        {unreadCount > 0 ? ` ${unreadCount} unread.` : " All caught up."}
      </p>
      <ul className="mt-6 space-y-3">
        {rows.map(({ job, last, unread }) => (
          <li key={job.id} className={`rounded-xl border bg-white p-4 ${unread ? "border-amber-300" : "border-stone-200"}`}>
            <div className="flex items-center justify-between gap-3">
              <Link href={`/dashboard/homeowner/jobs/${job.id}`} className="font-semibold text-stone-900 hover:underline">
                {job.title}
              </Link>
              {unread ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-900">Unread</span> : null}
            </div>
            <p className="mt-1 text-sm text-stone-700">
              {last ? `"${last.body.slice(0, 100)}${last.body.length > 100 ? "..." : ""}"` : "No messages yet — open the job to start."}
            </p>
            <p className="text-xs text-stone-500">
              {last ? `Last message ${new Date(last.createdAt).toLocaleString()}` : ""}
            </p>
            <Link href={`/dashboard/homeowner/jobs/${job.id}`} className="mt-2 inline-flex text-sm font-medium text-amber-800 hover:underline">
              Open thread
            </Link>
          </li>
        ))}
        {rows.length === 0 ? <li className="text-stone-600">No jobs to show messages for.</li> : null}
      </ul>
    </div>
  );
}
